...
if (isset($_POST['id'])) {
    ...
    $res=$mysqli->query("SELECT solution_id FROM solution WHERE user_id='".$_SESSION['user_id']."' AND "."problem_id = $id");
    if ($res->num_rows == 0){
        $res = $mysqli->query("SELECT set_name_show from problemset WHERE set_name='$set_name'");
        $set_name_show = $res->fetch_array()[0];
        $nowuser_level = $classList[0]; //获取用户最高能查看的题目类型 对应set_name_show
        if (isset($_SESSION['user_id'])){
            $sql = "SELECT class FROM users WHERE user_id='$uid'";
            $res = $mysqli->query($sql);
            $nowuser_level = $res->fetch_array()[0];
        }
        foreach ($classList as $class_now){
            if ($class_now == $set_name_show) break;
            if ($class_now == $nowuser_level) {
                $view_errors=  "Problem not available! And You Level is not enough!";
                require("template/".$OJ_TEMPLATE."/error.php");
                exit(0);
            }
        }
