...
error_reporting(E_ALL);
  ...
  /* 获取problemset中的对应关系 start*/
  $set_name_to_show_name = Array();
  $show_name_to_set_name = Array();

  $res_set = $mysqli->query("SELECT set_name,set_name_show FROM problemset");

  while ($res = $res_set->fetch_array()){
      $set_name_to_show_name[$res[0]] = $res[1];
      $show_name_to_set_name[$res[1]] = $res[0];
  }
  /* 获取problemset中的对应关系 end*/

  $who_open_this_page = $_SESSION['user_id'];
  // check user
  $user=$_GET['user'];
  ...
  $sql="SELECT `school`,`email`,`nick`,level,color,strength,real_name,class,stu_id,lv_system FROM `users` WHERE `user_id`='$user_mysql'";
  ...
  $stu_id=$row->stu_id;
  $class = $row->class;
  $show_class = $set_name_to_show_name[$class];
  $lv_system_number = $row->lv_system;
  ...
  if ($strength > $max_strength) {
    ...
    if ($strength < $level_strength[$j]) {
      $level = $level_name[$lv_system_number][$j-1];
