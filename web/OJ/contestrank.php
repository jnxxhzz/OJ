...
//init excluded_user => $is_excluded[$user_id]
...
if (!$user_limit) {
    $sql = "SELECT
            ...
            ORDER BY lv_system";
    $result = $mysqli->query($sql) or die($mysqli->error);
    while ($row=$result->fetch_object()) $classSet[] = $row->lv_system;
    $result->free();
}
else{
    $sql = "SELECT
...
$cls = $mysqli->real_escape_string($level_name_number[$_GET['lv_system']]); // class
