...
<?php
  ...
  $lv_system_name=$mysqli->real_escape_string(trim($_POST['level_system']));
  $class = 'default'; //默认选择最低的难度等级
  ...
  //获取等级体系对应id
  $lv_system = -1;
  if (isset($level_name_number[$lv_system_name]))
    $lv_system = $level_name_number[$lv_system_name];

  
  if ($lv_system == -1){
    $err_str=$err_str."Level_system Error\\n";
    $err_cnt++;
  }
  ...
  ."`user_id`,`email`,`ip`,`accesstime`,`password`,`reg_time`,`nick`,`school`,class, stu_id, real_name , lv_system)"
  ."VALUES('".$user_id."','".$email."','".$_SERVER['REMOTE_ADDR']."',NOW(),'".$password."',NOW(),'".$nick."','".$school."','".$class."','".$stu_id."','".$real_name."','".$lv_system."')";
